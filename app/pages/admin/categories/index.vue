<template>
  <section class="md:p-5 flex flex-col gap-6">
    ...
    >
      ...
      <template v-else>সংবাদ ধরন তালিকা</template>
    </h2>
      ...
      <tbody>
        ...
        <tr v-else-if="categories.length === 0">
          <td colspan="3" class="py-6 text-gray-400">
            ...
            <p class="text-xl">কোন ধরন নেই! নতুন ধরন তৈরি করুন।</p>
          </td>
        </tr>
...
import { Icon } from '@iconify/vue';
  ...
  const skeletonCount = computed(() =>
    categories.value.length > 0 ? categories.value.length : 5
  );
  ...
  async function confirmDelete() {
    ...
    try {
      await categoriesStore.deleteCategory(selectedCategory.id);
      categories.value = categoriesStore.categories;
      toast.success(
        `সংবাদ ধরন "${selectedCategory.name}" সফলভাবে মুছে ফেলা হয়েছে!`
      );
    } catch (err: any) {
      toast.error(err?.message || 'সংবাদ ধরন মুছতে ব্যর্থ হয়েছে!');
    } finally {
