<template>
  <div ref="dropdownRef" class="relative w-full">
    ...
    <button
      ...
      :class="['w-full h-12 rounded px-4 py-2 flex justify-between items-center gap-2 shadow-sm outline-none transition-all duration-150', error ? 'border-red-500 bg-red-50' : 'bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 dark:text-white focus:ring-1 focus:ring-green-500']"
    >
...
const toggleOption = (option: Option) => {
  ...
  if (exists) selected.value = selected.value.filter(s => s.value !== option.value)
...
const handleClickOutside = (event: MouseEvent) => {
  if (dropdownRef.value && !dropdownRef.value.contains(event.target as Node)) open.value = false
}
